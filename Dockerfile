...
ENV UPSTREAM_COMMIT=88e008ce0435f93c610bedf70a17b59159082f19
...
&& git clone --branch memory-fix https://github.com/vojkovic/searxng . \
...
IMAGE_PROXY=true PROXY= VALKEY_URL= REDIS_URL= LIMITER= BASE_URL= SECRET_KEY= CAPTCHA= AUTHORISED_API= MARGINALIA_API= JINA_API= BRAVE_API= NAME= SEARCH_DEFAULT_LANG= SEARCH_ENGINE_ACCESS_DENIED= SEARCH_ENGINE_CAPTCHA= ENGINE_TIMEOUT= PUBLIC_INSTANCE= \
...
LUXXLE_DEFAULT= ISEEK_DEFAULT= YANDEX_DEFAULT= YAHOO_DEFAULT= SWISSCOWS_DEFAULT= DOGPILE_DEFAULT= PRIVACYWALL_DEFAULT= VUHUV_DEFAULT= GMX_DEFAULT= DUCKDUCKGO_WEB_DEFAULT= RESULTHUNTER_DEFAULT= TUSKSEARCH_DEFAULT= GOOGLE_CSE_DEFAULT= \
OPENMETRICS= \
PRIVACYPOLICY= \
DONATE= \
API= \
